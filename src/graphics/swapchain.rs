...
impl Swapchain {
    pub fn create(base: &VkBase, window_size: PhysicalSize<u32>, present_mode: vk::PresentModeKHR) -> Self {
        let loader = swapchain::Device::new(&base.instance, &base.device);
        let (capabilities, format, present_mode) = Self::query_swap_chain_support(base, present_mode);
    ...
    pub fn recreate(&mut self, base: &VkBase, window_size: PhysicalSize<u32>) {
        ...
        let mut image_count = self.capabilities.min_image_count;
        ...
        self.inner = unsafe { self.loader.create_swapchain(&create_info, None).unwrap_unchecked() };
        self.image_views = Self::create_image_views(&self.loader, &self.inner, &base.device, self.format.format);
    }

    fn query_swap_chain_support(base: &VkBase, present_mode: vk::PresentModeKHR) -> (SurfaceCapabilitiesKHR, vk::SurfaceFormatKHR, vk::PresentModeKHR){
        ...
        let format: vk::SurfaceFormatKHR = unsafe {
            base.surface_loader.get_physical_device_surface_formats(base.physical_device, base.surface).unwrap_unchecked().into_iter().find(|format| {format.format == vk::Format::R8G8B8A8_SRGB && format.color_space == vk::ColorSpaceKHR::SRGB_NONLINEAR}).unwrap()
        };

        let present_mode = unsafe { 
            base.surface_loader.get_physical_device_surface_present_modes(base.physical_device, base.surface).unwrap_unchecked().into_iter().find(|pm| {*pm == present_mode}).unwrap()
        };
    ...
    pub fn destroy(&mut self, device: &Device, framebuffer: &Vec<Framebuffer>) {
        unsafe  {
